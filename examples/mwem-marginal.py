...
A second minor enhancement, not described in 

"""

def worst_approximated(workload_answers, est, workload, eps):
    ...
    for cl in workload:
        ...
        errors = np.append(errors, np.abs(x - xest).sum())
    errors = np.array(errors)
    prob = softmax(0.5*eps*(errors - errors.max()))
    key = np.random.choice(len(errors), p=prob)
    return workload[key]

def mwem_pgm(data, epsilon, delta=0.0, workload=None, rounds=None, maxsize_mb = 25):
    """
    ...
    engine = FactoredInference(data.domain, log=False, iters=100, warm_start=True)
    measurements = []
