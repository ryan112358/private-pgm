...
from mbi.einsum import scan_einsum
import numpy as np
...
class TestScanEinsum(unittest.TestCase):
    ...
    def test_no_sequential(self):
        ...
        expected_complex = jnp.einsum("ij,jk,il->kl", A, B, C) # A(2,3) B(3,4) C(2,4) -> (4,4)
    ...
    def test_single_sequential_summed_out(self):
        ...
        self.assertArraysAllClose(expected_xy, actual_xy_j, msg="Sequential 'j' with size 1")
        actual_xy_k = scan_einsum("ijk,jkl->il", X, Y, sequential="k") # k is size 3
        self.assertArraysAllClose(expected_xy, actual_xy_k, msg="Sequential 'k' with size 3")
    ...
    def test_single_sequential_in_output(self):
        ...
        self.assertArraysAllClose(expected_broadcast, actual_broadcast_i, msg="Broadcast seq 'i'")
        actual_broadcast_j = scan_einsum("i,j->ij", x, y, sequential="j")
        self.assertArraysAllClose(expected_broadcast, actual_broadcast_j, msg="Broadcast seq 'j'")
    ...
    def test_multiple_sequential_axes(self):
        ...
        self.assertArraysAllClose(expected_xy, actual_xy_j, msg="Sequential 'j' for ijk,j->ik")
        ...
        self.assertArraysAllClose(expected_abc, actual_abc_jk, msg="Sequential 'jk' for aij,bjk,ckl->abil")

        # Seq "ab" (in output)
        actual_abc_ab = scan_einsum("aij,bjk,ckl->abil", T1, T2, T3, sequential="ab")
        self.assertArraysAllClose(expected_abc, actual_abc_ab, msg="Sequential 'ab' for aij,bjk,ckl->abil")

        # Seq "aj" (a in output, j summed out)
        actual_abc_aj = scan_einsum("aij,bjk,ckl->abil", T1, T2, T3, sequential="aj")
        self.assertArraysAllClose(expected_abc, actual_abc_aj, msg="Sequential 'aj' for aij,bjk,ckl->abil")
        ...
        self.assertArraysAllClose(expected_all_seq, actual_all_seq_ijk, msg="Seq 'ijk' all summed")
        actual_all_seq_ikj = scan_einsum("ijk,jkl,kli->", D1,D2,D3, sequential="ikj") # order change
        self.assertArraysAllClose(expected_all_seq, actual_all_seq_ikj, msg="Seq 'ikj' all summed")
    ...
    def test_sequential_axis_not_in_all_arrays(self):
        ...
        self.assertArraysAllClose(expected_xyz, actual_a, msg="Seq 'a' for ab,bc,cd->ad")
        # Seq "b": X,Y sliced, Z whole
        actual_b = scan_einsum("ab,bc,cd->ad", X,Y,Z, sequential="b")
        self.assertArraysAllClose(expected_xyz, actual_b, msg="Seq 'b' for ab,bc,cd->ad")
        # Seq "c": Y,Z sliced, X whole
        actual_c = scan_einsum("ab,bc,cd->ad", X,Y,Z, sequential="c")
        self.assertArraysAllClose(expected_xyz, actual_c, msg="Seq 'c' for ab,bc,cd->ad")
        # Seq "d": Z sliced, X,Y whole
        actual_d = scan_einsum("ab,bc,cd->ad", X,Y,Z, sequential="d")
        self.assertArraysAllClose(expected_xyz, actual_d, msg="Seq 'd' for ab,bc,cd->ad")
        # Seq "ac": a in X, c in Y,Z
        actual_ac = scan_einsum("ab,bc,cd->ad", X,Y,Z, sequential="ac")
        self.assertArraysAllClose(expected_xyz, actual_ac, msg="Seq 'ac' for ab,bc,cd->ad")
