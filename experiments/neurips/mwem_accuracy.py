...
from mbi import Factor, FactoredInference, RegionGraph
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', choices=['MW','MB','CVX'], help='inference engine')
    ...
    data, _, workloads = benchmarks.random_hdmm(args.dataset, args.workload)
    ...
    if args.engine == 'MW':
        ...
        cliques = [cl for cl, W in workloads]
        model = RegionGraph(data.domain, cliques, iters=1, minimal=True)
        engine = FactoredInference(data.domain, iters=args.iters, marginal_oracle=model)
    ...
    path = 'results/mwem.csv'

    with open(path, 'a') as f:
        f.write('%s,%s,%s,%s,%s \n' % (args.dataset,args.seed,args.epsilon,args.engine,error))
